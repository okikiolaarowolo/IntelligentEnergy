...
type Run = { id: string; scenario_id: string; status: string; simulator_version: string; input_snapshot: any; result_snapshot: Result; created_at: string; completed_at: string | null };
...
export default function Dashboard() {
  ...
  useEffect(() => { if (selectedScenario) loadRuns(selectedScenario.id).catch((e) => setError(e.message)); }, [scenarioId]);
  ...
  async function createScenario(e: FormEvent) {
    ...
    try {
      ...
      const body = {
        projectId, name: scenarioName, generationProfile: parseProfile(generation, 'Generation profile'), demandProfile: parseProfile(demand, 'Demand profile'),
        batteryCapacityKwh: Number(capacity), initialSocKwh: Number(soc), chargeEfficiency: Number(chargeEfficiency), dischargeEfficiency: Number(dischargeEfficiency),
      };
  ...
  async function deleteScenario(id: string) {
    if (!confirm('Delete this scenario and its simulation history?')) return;
    setError('');
  ...
  async function deleteProject() {
    if (!projectId || !confirm('Delete this project, its scenarios, and simulation history?')) return;
    setError('');
  ...
  async function signOut() {
    ...
    await createClient().auth.signOut(); window.location.href = '/login';
  }

  return <>
    <nav className="nav"><strong>IntelligentEnergy</strong><button onClick={signOut}>Sign out</button></nav>
    <main className="container">
      ...
      <div className="grid grid-3">
        <section className="card">
          <h2>Project</h2>
          <form onSubmit={createProject}><input placeholder="New project name" value={projectName} onChange={(e) => setProjectName(e.target.value)} required maxLength={120} /><button disabled={busy}>Create project</button></form>
          <select value={projectId} onChange={(e) => setProjectId(e.target.value)} aria-label="Select project"><option value="">Select project</option>{projects.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}</select>
          {projectId && <button className="danger" onClick={deleteProject}>Delete project</button>}
        </section>

        <section className="card">
          <h2>Scenarios</h2>
          <select value={scenarioId} onChange={(e) => { setScenarioId(e.target.value); const s = scenarios.find((x) => x.id === e.target.value); if (s) { setGeneration(s.generation_profile.join(',')); setDemand(s.demand_profile.join(',')); setCapacity(String(s.battery_capacity_kwh)); setSoc(String(s.initial_soc_kwh)); setChargeEfficiency(String(s.charge_efficiency)); setDischargeEfficiency(String(s.discharge_efficiency)); setScenarioName(s.name); } }} aria-label="Select scenario"><option value="">Select scenario</option>{scenarios.map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}</select>
          {selectedScenario && <div className="actions"><button onClick={() => runScenario(selectedScenario.id)} disabled={busy}>Run selected</button><button className="danger" onClick={() => deleteScenario(selectedScenario.id)}>Delete</button></div>}
        </section>
      ...
      <section className="card form-card">
        <h2>Create / update scenario configuration</h2>
        <form onSubmit={createScenario} className="form-grid">
          <label>Scenario name<input value={scenarioName} onChange={(e) => setScenarioName(e.target.value)} maxLength={120} required /></label>
          <label>Generation profile (kWh/step)<input value={generation} onChange={(e) => setGeneration(e.target.value)} /></label>
          <label>Demand profile (kWh/step)<input value={demand} onChange={(e) => setDemand(e.target.value)} /></label>
          <label>Battery capacity (kWh)<input type="number" min="0" step="any" value={capacity} onChange={(e) => setCapacity(e.target.value)} /></label>
          <label>Initial SOC (kWh)<input type="number" min="0" step="any" value={soc} onChange={(e) => setSoc(e.target.value)} /></label>
          <label>Charge efficiency<input type="number" min="0.000001" max="1" step="any" value={chargeEfficiency} onChange={(e) => setChargeEfficiency(e.target.value)} /></label>
          <label>Discharge efficiency<input type="number" min="0.000001" max="1" step="any" value={dischargeEfficiency} onChange={(e) => setDischargeEfficiency(e.target.value)} /></label>
          <button disabled={busy || !projectId}>{busy ? 'Working…' : 'Save new scenario'}</button>
        </form>
      </section>

      {result && <>
        <section className="card"><h2>Simulation results</h2><div className="metrics">{Object.entries(result.totals).map(([key, value]) => <div className="metric" key={key}><span>{key}</span><strong>{Number(value).toFixed(3)} kWh</strong></div>)}</div></section>
        <div className="grid grid-2">
          <section className="card chart-card"><h2>Generation vs demand</h2><ResponsiveContainer width="100%" height={300}><BarChart data={chartData}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="step" /><YAxis /><Tooltip /><Legend /><Bar dataKey="generation" name="Generation" /><Bar dataKey="demand" name="Demand" /></BarChart></ResponsiveContainer></section>
          <section className="card chart-card"><h2>Battery state of charge</h2><ResponsiveContainer width="100%" height={300}><LineChart data={chartData}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="step" /><YAxis /><Tooltip /><Legend /><Line type="monotone" dataKey="soc" name="SOC (kWh)" /></LineChart></ResponsiveContainer></section>
        </div>
        <section className="card"><h2>Time-step detail</h2><div className="table-wrap"><table><thead><tr><th>Step</th><th>Generation</th><th>Demand</th><th>Direct to load</th><th>Charge</th><th>Discharge</th><th>Curtailed</th><th>Unmet</th><th>SOC</th></tr></thead><tbody>{result.steps.map((s, i) => <tr key={i}><td>{i + 1}</td><td>{s.generation.toFixed(3)}</td><td>{s.demand.toFixed(3)}</td><td>{s.directGenerationToLoad.toFixed(3)}</td><td>{s.batteryChargeInput.toFixed(3)}</td><td>{s.batteryDischargeDelivered.toFixed(3)}</td><td>{s.curtailed.toFixed(3)}</td><td>{s.unmet.toFixed(3)}</td><td>{s.socKwh.toFixed(3)}</td></tr>)}</tbody></table></div></section>
      </>}
    </main>
  </>;
}
